        ...
        suspend fun insertRate(
            ...
            when (service.serviceType) {
                ...
                ServiceType.ELECTRICITY -> {
                    ...
                    rates.add(RateEntity.electricityRateFrom800(service.serviceId, payerServiceId))
                    rate = RateEntity.electricityPrivilegesRate(service.serviceId, payerServiceId)
                }
                ServiceType.GAS -> rate = RateEntity.gasRate(service.serviceId, true, isPrivileges)
                ServiceType.COLD_WATER ->
                    rate = RateEntity.coldWaterRate(service.serviceId, isPerPerson, isPrivileges)
                ServiceType.WASTE ->
                    rate = RateEntity.wasteRate(service.serviceId, isPerPerson, isPrivileges)
                ServiceType.HEATING ->
                    rate = RateEntity.heatingRate(service.serviceId, isPerPerson, isPrivileges)
                ServiceType.HOT_WATER ->
                    rate = RateEntity.hotWaterRate(service.serviceId, isPerPerson, isPrivileges)
                // Payer service rates:
                ServiceType.RENT ->
                    payerServiceId?.let {
                        rate = RateEntity.rentRateForPayer(
                            service.serviceId, it, isPerPerson, isPrivileges
                        )
                    }
                ServiceType.GARBAGE ->
                    payerServiceId?.let {
