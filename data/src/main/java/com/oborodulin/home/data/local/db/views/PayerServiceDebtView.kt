...
SELECT psd.*, 
    ...
    (CASE WHEN psd.isPerPerson = 1
            ...
            THEN printf(${Constants.FMT_IS_PER_PERSON_EXPR}, psd.personsNum, psd.personMu, 
                            psd.rateValue / ${CONV_COEFF_BIGDECIMAL}.0, psd.currencyCode, psd.serviceDebt / ${CONV_COEFF_BIGDECIMAL}.0, psd.currencyCode)
            ELSE CASE 
                    -- RENT
                    WHEN psd.serviceType IN (${Constants.SRV_RENT_VAL}) THEN 
                        (CASE WHEN psd.totalArea IS NOT NULL    
                            ...
                            ELSE '' 
                        END) || 
                            printf(${Constants.FMT_RATE_DEBT_EXPR}, psd.rateValue / ${CONV_COEFF_BIGDECIMAL}.0, psd.currencyCode, psd.serviceDebt / ${CONV_COEFF_BIGDECIMAL}.0, psd.currencyCode)                        
                    -- HEATING
                    ...
                    ELSE printf(${Constants.FMT_DEBT_EXPR}, psd.serviceDebt / ${CONV_COEFF_BIGDECIMAL}.0, psd.currencyCode)
                END
    END) serviceDebtExpr
FROM (SELECT rps.payerId, rps.personsNum, rps.totalArea, rps.livingSpace, 
        ifnull(rps.fromServiceDate, ifnull(mrv.paymentDate, strftime(${Constants.DB_FRACT_SEC_TIME}, 'now', 'local', 'start of month'))) AS paymentDate, 
        ifnull(rps.fromServiceMonth, ifnull(mrv.paymentMonth, CAST(strftime('%m', strftime(${Constants.DB_FRACT_SEC_TIME}, 'now', 'local', 'start of month')) AS INTEGER))) AS paymentMonth, 
        ifnull(rps.fromServiceYear, ifnull(mrv.paymentYear, CAST(strftime('%Y', strftime(${Constants.DB_FRACT_SEC_TIME}, 'now', 'local', 'start of month')) AS INTEGER))) AS paymentYear, 
        rps.isPerPerson, rps.servicePos, rps.serviceType, rps.serviceName, rps.fromServiceDate,
        rps.serviceLocCode, rps.startDate AS rateStartDate, rps.rateValue, 
    ...
    -- payer services with fromServiceDate controls by its receipts
        LEFT JOIN ${ReceiptView.VIEW_NAME} rcv ON rcv.payersId = rps.payerId 
                                                AND rcv.receiptMonth = rps.fromServiceMonth 
                                                AND rcv.receiptYear = rps.fromServiceYear
...
SELECT psd.*, 
    ...
    (CASE WHEN psd.isDerivedUnit = 0
            THEN printf(${Constants.FMT_METER_VAL_EXPR}, psd.rateMeterValue / ${CONV_COEFF_BIGDECIMAL}.0, psd.measureUnit,
                                                        ...
                                                        psd.serviceDebt / ${CONV_COEFF_BIGDECIMAL}.0, psd.currencyCode)                        
...
class PayerServiceDebtView(
    ...
    val rateStartDate: OffsetDateTime,
